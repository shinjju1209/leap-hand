...
## 11. 절대 무적 가위바위보 AI (100% 승률 비전 카운터, `rps_unbeatable_game.py`)

도쿄대학교 이시카와 연구소(Ishikawa Lab)의 100% 승률 쟝켄 로봇 원리를 적용한 초고속 비전 카운터 가위바위보 독립 실행형 프로그램입니다.

```bash
# 1. 일반 웹캠 GUI 모드 (하드웨어 없이 웹캠 화면만으로 AI 대결)
python rps_unbeatable_game.py --mode gui

# 2. 실물 LEAP Hand 하드웨어 연동 100% 승리 대결
python rps_unbeatable_game.py --mode hardware --port /dev/ttyUSB0

# 3. 실물 하드웨어 + MuJoCo 3D 디지털 트윈 동시 실행
python rps_unbeatable_game.py --mode both --port /dev/ttyUSB0
```

### 🎮 주요 특징 및 모드

1. **😈 GOD MODE (100% 승리 무적 AI)**:
   * 카운트다운(3-2-1 탕!) 순간 사람이 손을 뻗는 찰나(약 15~20ms)에 MediaPipe로 사람 패를 실시간 읽고, **무조건 이기는 상성 패를 즉시 발사**하여 100% 승리합니다.
2. **🎲 FAIR MODE (공정 대결)**: 순수 무작위 수로 대결하는 공정 모드.
3. **🤡 VIP / TROLL MODE (100% 패배)**: 사람에게 무조건 져주는 접대 모드.
4. **🪞 MIRROR MODE (100% 무승부)**: 사람 손을 그대로 복사하는 미러 모드.
5. **실시간 텔레메트리**: 비전 AI 반응 속도(ms 단위 정밀 측정), 연승 스트릭(Streak), 스코어보드 통계 실시간 표시.
6. **단축키**: `[Space]` 라운드 시작 / `[M]` 모드 변경 / `[P]` 자동 연속 대결 토글 / `[R]` 점수 초기화 / `[Q]` 종료.
